...
Запрос возвращает массив с коллекциями, не более 20 коллекций. Далее, если в ответе 20 коллекций, то это означает, что у владельца могут быть еще коллекции. Чтобы получить еще коллекции, можно повторить запрос, указав `start_name` - `name` 20-й коллекции.
...
Кроме того, `creator` можно и не указывать, тогда будут выдаваться все NFT-коллекции в системе.

Также запрос поддерживает параметры `filter_creators` (массив - черный список имен владельцев, чьи коллекции не стоит возвращать в ответе) и `filter_names` (массив - черный список идентификаторов коллекций)
